...
```{r 6.5.1a Quick Check on Day to Night Ramp Verification: aTemperature Values, All MOATs}
ggplot(subset(dml, 
               period %in% ("D2Nramp")), 
...
```{r 6.5.1b Quick Check on Day to Night Ramp Verification aTemperature Values, All Change}

ggplot (dml %>%
  filter(dml$period = "day" | "D2Nramp",
         dml$treatment = "allchange"),
        aes(x= dml$dateTime, y= dml$aTemperature)) + 
   geom_point(aes(colour= dml$period, point=)) +
   ylim (5.0, 15.00) +
   ggtitle("aTemperature Values, All Change Treatment, Day Ramp Cycle Day to Night")
