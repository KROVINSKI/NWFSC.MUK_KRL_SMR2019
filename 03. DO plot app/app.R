...
##looking at the October 2019 SPEC app verison to better inform and create a DO timeseries App
...
# Define UI ----
ui <- fluidPage(
  titlePanel("DO Analysis"),
  sidebarLayout(
    sidebarPanel( width = 2,
                  fileInput("files", h4("DO Input Files"), multiple = TRUE, accept = c(".xls", ".xlsx")),
                  ...
                  checkboxGroupInput("colourBy", h4("Colour By"), 
                                     choices = c("None", "MOATS", "Treatment",
                                     selected = "None"),
                  radioButtons("facetBy", h4("Facet By"), 
                               choices = c("None", "MOATS", "Treatment",
                               selected = "None"),
                  uiOutput("MOATSInclude"),
                  uiOutput("Treatmentinclude"),
                  sliderInput("ySlider", "Y-axis Range)", 
                              min = 2, max = 10, value = c(2, 10), step = 0.1),
                  checkboxInput("yRangeCheckbox", "Limit Graph Y-axixs Range", value = FALSE),
                  dateRangeInput("dates", h4("Date range"), start = "2019-08-01"),
...
#   
#   ################
#   #read in DO data files
#   # combines all the DO files into a single data set
#   values$specData <- observeEvent(input$files, {
#     
# ##Data frame needs to include MOATS, treatment, sensor (the presens or "SystemProbe")
#     # d <- data.frame(MOATs = character(0), Treatment = character(0), Sensor  = character(0),
#     #                 stringsAsFactors = FALSE)
#     d <- NULL
#     for(i in 1:length(input$files$name)){
#       # Set the progress bar, and update the detail text.
#       dTemp <- read_excel(input$files$datapath[i], sheet = "DO sheet", skip = 1, col_types = "text")
