...
# # "M01thruM13moatslog"
write.csv(M01thruM13moatslog_data, file = "M01thruM13moatslog_n17.csv", row.names = FALSE)
...
wcdml <- read.csv( file = "M01thruM13moatslog_n17.csv", stringsAsFactors = FALSE)
dim(wcdml)
#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
wcdml <- wcdml %>% arrange(moats, dateTime) %>% filter(row_number() %% 17 == 0)

write.csv(wcdml, file = "M01thruM13moatslog_n17.csv", row.names = FALSE)
#|- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
